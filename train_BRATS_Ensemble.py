...
try:
    from .models.unet_Ensemble import UNet_Ensemble
...
def train_epoch(net, train_loader, config):
    ...
    for batch in tqdm.tqdm(train_loader):
        ...
        img = img.cuda()
        mask = mask.cuda()
        soft_out, _, kl = net.forward(img, config.num_train_samples, config.num_classes)
        # calculate headwise loss
        l_dis = 0.0
        for ens in range(soft_out.size(0)):
            l_dis += (1 - config.LAMBDA) * (loss(torch.log(soft_out[ens, :, :] + 1e-18), mask) + config.klfactor * kl[
                ens]) / config.NUM_MODELS

        # make ensemble prediction
        soft = F.softmax(soft_out.sum(0), dim=1)
        preds = torch.argmax(soft, 1)
        l = loss(torch.log(soft + 1e-18), mask)

        if torch.isnan(l).any() or torch.isinf(l).any():
            print("softmax", soft_out.size(), soft_out.min().item(), soft_out.max().item())
            print("preds", preds.size(), preds.unique())
            print("loss", l.item(), l_dis)

        # Sum whole loss
        l_final = config.NUM_MODELS * config.LAMBDA * (l + config.klfactor * torch.sum(kl)) + l_dis

        l_final.backward()
        opti.step()

        soft_out = soft.detach().cpu()
        ...
        mean_train_loss += l
        mean_train_kl += torch.sum(kl)

        # Compute the entropy of the prediction!
        mean_train_entropy -= torch.sum(
            torch.sum(soft_out / config.NUM_MODELS * torch.log(soft_out / config.NUM_MODELS + 1e-18), 1)[un255])
...
def eval_epoch(net, val_loader, writer, step, config):
    ...
    conf_mat_eval = ConfMatrix(config.num_classes, config.ignore_index, cuda=config.cm_cuda)
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm.tqdm(val_loader)):
            ...
            if i != len(val_loader) - 2:
                soft_out, _, kl = net.forward(img, config.num_eval_samples, config.num_classes)
            else:
                soft_out, _, kl = net.forward(img, config.num_plot_samples, config.num_classes)

            soft_out = soft_out.sum(0)
            preds = torch.argmax(soft_out, 1)

            l = loss(torch.log(soft_out / config.NUM_MODELS + 1e-18), mask)
            ...
            num_eval_pixels += (mask.size(0) * mask.size(1) * mask.size(2)) - torch.sum(mask == 255).item()

            mean_eval_loss += l
            mean_eval_kl += torch.sum(kl)

            # Compute the entropy of the prediction!
            mean_eval_entropy -= torch.sum(
                torch.sum(soft_out / config.NUM_MODELS * torch.log(soft_out / config.NUM_MODELS + 1e-18), 1)[un255])

            conf_mat_eval.addPred(mask, preds)

            if i == len(val_loader) - 2:
                uncert = -torch.sum((soft_out / config.NUM_MODELS) * torch.log((soft_out / config.NUM_MODELS)),
                                    1) / torch.log(torch.Tensor([config.num_classes]))
                plot_prediction(preds, uncert, img, mask, val_loader.dataset, writer, step)
            break
...
if __name__ == "__main__":
    ...
    net = UNet_Ensemble(num_models=config.NUM_MODELS, mutliHead_layer=config.model_type, num_in=config.num_channels,
